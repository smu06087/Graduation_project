...
Connection con = DriverManager.getConnection
		(dbUrl, dbUser, dbPass);
String sql = "SELECT FLOOR((sum_num - 1) / 3600) as group_num, AVG(sum_count) as avg_sum_count FROM Ex_sum_count GROUP BY group_num LIMIT 6 OFFSET 1;";
...
String sql1 = "select * from Ex_sum_count1 ORDER BY sum_num DESC LIMIT 1; ";
PreparedStatement pstmt1 = con.prepareStatement(sql1);
ResultSet rs1 = pstmt1.executeQuery(sql1);

String sql2 = "select * from west_cal ORDER BY west_num DESC LIMIT 1;";
                     ...
                     <figure>
                        ...
                        <div class="portfolio_text" style="background-color: #E8E8E8;">
                           ...
                           <%
								...
								%>
								이동 소요 시간 <br>
								동쪽 : <%=rs4.getString("opp")%>분 
								남쪽 : <%=rs4.getString("next")%>분 
								북쪽 : <%=rs4.getString("next")%>분<br>
								<%
								}
								%>
								...
								while (rs2.next()) {
								%>
								추천 출입구 : <%=rs2.getString("wrec")%>합니다<br>
								<%
								}
								%>
                        </div>
